...
// Public: Get approved companies (no authentication required)
exports.getPublicCompanies = async (req, res) => {
  try {
    const { search, page = 1, size = 10, category, minRating } = req.query;
    const { limit, offset } = getPagination(page, size);
    const searchTerm = escapeRegex(search);

    const query = {
      status: "approved", // Only show approved companies to public
    };
    ...
    if (minRating) {
      ...
      filteredCompanies = companies.filter(
        (c) => c.ratingsAggregate >= minRatingNum
      );
    }
    ...
    const totalItems = await CompanyModel.countDocuments(query);
    const finalTotal = minRating ? filteredCompanies.length : totalItems;
...
exports.createCompany = async (req, res) => {
  try {
    ...
    if (req.files && req.files.logo) {
      ...
      const url = await r2.uploadPublic(
        logo.path,
        `${logo.filename}`,
        "Companies"
      );
...
exports.updateCompany = async (req, res) => {
  try {
    ...
    // Check permissions
    if (
      role === constants.roles.operator &&
      company.operatorId.toString() !== userId.toString()
    ) {
      return sendErrorResponse(
        res,
        "You can only update your own companies",
        403,
        true,
        true
      );
    ...
    if (role !== constants.roles.operator && role !== constants.roles.admin) {
      return sendErrorResponse(
        res,
        "Only operators and admins can update companies",
        403,
        true,
        true
      );
    ...
    if (
      req.body.ratingsAggregate !== undefined ||
      req.body.totalReviews !== undefined
    ) {
...
exports.deleteCompany = async (req, res) => {
  try {
    ...
    if (role !== constants.roles.operator && role !== constants.roles.admin) {
      return sendErrorResponse(
        res,
        "Only operators and admins can delete companies",
        403,
        true,
        true
      );
...
exports.addPromoCode = async (req, res) => {
  try {
    ...
    if (
      ...
      return sendErrorResponse(
        res,
        "You can only add promo codes to your own companies",
        403,
        true,
        true
      );
    ...
    if (role !== constants.roles.operator && role !== constants.roles.admin) {
      return sendErrorResponse(
        res,
        "Only operators and admins can add promo codes",
        403,
        true,
        true
      );
...
exports.updatePromoCode = async (req, res) => {
  try {
    ...
    if (
      ...
      return sendErrorResponse(
        res,
        "You can only update promo codes for your own companies",
        403,
        true,
        true
      );
    ...
    if (role !== constants.roles.operator && role !== constants.roles.admin) {
      return sendErrorResponse(
        res,
        "Only operators and admins can update promo codes",
        403,
        true,
        true
      );
...
exports.deletePromoCode = async (req, res) => {
  try {
    ...
    if (
      ...
      return sendErrorResponse(
        res,
        "You can only delete promo codes from your own companies",
        403,
        true,
        true
      );
    ...
    if (role !== constants.roles.operator && role !== constants.roles.admin) {
      return sendErrorResponse(
        res,
        "Only operators and admins can delete promo codes",
        403,
        true,
        true
      );
